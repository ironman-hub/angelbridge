...
const caseSchema = z.object({
  ...
  needsOther: z.string().optional().default(""),
  have: z.array(z.string()).default([]),
  isSafe: z.boolean({ required_error: "Please tell us whether you are safe" }),
  unsafeReason: z.string().optional().default(""),
  isInjured: z.boolean({ required_error: "Please tell us whether you are injured" }),
  hasOthers: z.boolean({ required_error: "Please tell us whether anyone is with you" }),
  othersWith: z.array(z.string()).default([]),
  contactedHelp: z.boolean({ required_error: "Please tell us whether you have contacted anyone" }),
  contactedWho: z.array(z.string()).default([]),
  waitingFor: z.string().optional().default(""),
  estimatedWait: z.string().optional().default(""),
  canBuyFood: z.boolean({ required_error: "Please answer whether you can buy food" }),
  safeTonight: z.boolean({ required_error: "Please answer whether you have somewhere safe tonight" }),
  previousHelp: z.boolean({ required_error: "Please answer whether we have helped you before" }),
  shareIdentity: z.boolean().default(true),
  carReg: z.string().optional().default(""),
  surroundings: z.string().optional().default(""),
  wearing: z.string().optional().default(""),
  ...
  consentEmergency: z.literal(true, { errorMap: () => ({ message: "Please acknowledge the emergency-services statement" }) }),
});
...
export const POST = handler(async (req: NextRequest) => {
  ...
  const created = await prisma.case.create({
    data: {
      ...
      needsOther: data.needsOther || null,
      ...
      canBuyFood: data.canBuyFood,
      safeTonight: data.safeTonight,
      shareIdentity: data.shareIdentity,
      carReg: data.shareIdentity ? (data.carReg || null) : null,
      surroundings: data.shareIdentity ? (data.surroundings || null) : null,
      wearing: data.shareIdentity ? (data.wearing || null) : null,
      previousHelp: data.previousHelp,
      idVerified: data.idVerified,
      consentEmergency: data.consentEmergency,
  ...
  if (result.recommendedStatus === "Approved") {
    events.push({ type: "dispatched", message: `Nearest Angel Bridge Foundation responder notified, estimated arrival ${etaMinutes} min` });
  }
